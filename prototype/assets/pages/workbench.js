/* ───────── 标注工作台(核心页) ─────────
   设计要点:
   - 表格渲染:一次性 innerHTML;事件用委托,避免逐行 addEventListener
...
(function () {
  ...
  const state = {
    ...
    search: '',
    contrastRole: null,
    rowFilterFromMetric: null,  // 指标点击后的临时筛选
    drawerRowId: null,
    drawerTab: 'current',
  };
  ...
  /* ───────── Init toolbar / inner tabs ───────── */
  function initToolbar () {
    const dsSel = document.getElementById('tb-dataset');
    NX.datasets.filter(d => d.mappingDone).forEach(d => {
      const o = document.createElement('option'); o.value = d.id; o.textContent = `${d.name} (${d.scene})`;
      dsSel.appendChild(o);
    });
    ...
    document.getElementById('tb-start').addEventListener('click', () => {
      ...
      if (sel.length === 0) {
        NX.toast('请先勾选要标注的行,或在批量栏点击「标注选中」', 'default'); return;
      }
      simulateAnnotate(sel);
    });
  }

  function initSchemeAndDataset () {
    // 默认选第一个 mappingDone 的数据集
    const firstDs = NX.datasets.find(d => d.mappingDone);
    if (firstDs) {
      state.dsId = firstDs.id;
      document.getElementById('tb-dataset').value = firstDs.id;
    }
    refreshScheme();
  }
  ...
  function refreshScheme () {
    ...
    NX.schemes.forEach(s => {
      ...
      const promptCnt = s.promptIds.length;
      o.textContent = `${s.name} · ${promptCnt} Prompt`;
      sel.appendChild(o);
    });
  ...
  function refreshContrastRoles () {
    ...
    sch.promptIds.forEach(pid => {
      const p = NX.prompts.find(x => x.id === pid);
      if (!p) return;
      const o = document.createElement('option'); o.value = p.role; o.textContent = `对照:${p.role}`;
      sel.appendChild(o);
    });
    state.contrastRole = sch.promptIds.length > 0
      ? NX.prompts.find(x => x.id === sch.promptIds[0])?.role
      : null;
  ...
  function bindInnerTabs () {
    document.querySelectorAll('.tab-inner').forEach((tab) => {
      tab.addEventListener('click', () => {
        document.querySelectorAll('.tab-inner').forEach(t => t.classList.remove('active', 'font-semibold'));
        document.querySelectorAll('.tab-inner').forEach(t => t.classList.add('font-medium'));
        ...
        ['annotate', 'schemes', 'tasks'].forEach(k => {
          ...
          if (k === key) p.classList.remove('hidden'); else p.classList.add('hidden');
          if (k === key) p.classList.add('flex'); else p.classList.remove('flex');
        });
  ...
  function bindViewTabs () {
    document.querySelectorAll('.tab-view').forEach(tab => {
      tab.addEventListener('click', () => {
        document.querySelectorAll('.tab-view').forEach(t => t.classList.remove('active', 'font-semibold'));
        document.querySelectorAll('.tab-view').forEach(t => t.classList.add('font-medium'));
        tab.classList.add('active', 'font-semibold');
        tab.classList.remove('font-medium');
    ...
    if (state.view !== 'all') {
      rows = rows.filter(r => {
        ...
        if (state.view === 'mismatch') {
          ...
          return r.results['[初审]_情感分类'] !== r.data['情感分类'];
        }
        return true;
      });
    }
  ...
  /* TP/TN/FP/FN 分类 - 以"正面"为正类 */
  function classify (r) {
    if (r.status !== 'done') return null;
    const role = state.contrastRole || '初审';
    const pred = r.results[`[${role}]_情感分类`];
    const gt   = r.data['情感分类'];
    if (pred == null || gt == null) return null;
    const POS = '正面';
    if (pred === POS && gt === POS) return 'tp';
    if (pred !== POS && gt !== POS) return 'tn';
    if (pred === POS && gt !== POS) return 'fp';
    if (pred !== POS && gt === POS) return 'fn';
    return null;
  }
  ...
  function renderMetricBar () {
    ...
    const cnt = { done:0, running:0, pending:0, failed:0, partial:0 };
    rows.forEach(r => cnt[r.status]++);

    const mb1 = document.getElementById('mb-row1');
    mb1.innerHTML = [
    ...
    document.getElementById('mb-confusion').innerHTML = [
    ...
    document.getElementById('mb-metrics').innerHTML = [
      pctChip('准确率', acc),
      pctChip('精确率', prec),
      pctChip('召回率', rec),
      pctChip('F1',     f1),
    ...
    document.querySelectorAll('[data-metric]').forEach(el => {
      el.addEventListener('click', () => {
        ...
        renderTable();
        updateBatchBar();
        document.querySelectorAll('[data-metric]').forEach(b => b.classList.remove('ring-2', 'ring-offset-2'));
        if (state.rowFilterFromMetric) el.classList.add('ring-2', 'ring-emerald-400', 'ring-offset-2');
      });
    });
  }
  ...
  function metricChip (label, value, color, key) {
    ...
    return `<button ${key ? `data-metric="${key}"` : ''} class="inline-flex items-center gap-2 rounded-lg ${cls} px-3 py-1.5 text-sm transition-colors ${key ? 'cursor-pointer' : 'cursor-default'}">
      <span class="font-medium">${label}</span>
      <span class="text-base font-semibold">${value.toLocaleString()}</span>
    </button>`;
  }
  function confChip (label, value, color, key) {
    ...
    return `<button data-metric="${key}" class="inline-flex items-center gap-2 rounded-lg ${colors[color]} px-2.5 py-1 text-sm transition-colors cursor-pointer">
  ...
  function getCurrentScheme () {
    return NX.schemes.find(s => s.id === state.schemeId);
  }
  function getCurrentPrompts () {
    const sc = getCurrentScheme();
    if (!sc) return [];
    return sc.promptIds.map(pid => NX.prompts.find(p => p.id === pid)).filter(Boolean);
  }
  function getCurrentDataset () {
    return NX.datasets.find(d => d.id === state.dsId);
  }
  function getDisplayCols () {
    const d = getCurrentDataset();
    if (!d || !d.mapping) return NX.workbenchDefaultCols || ['ID', '用户反馈'];
    return d.mapping.defaultCols;
  }
  ...
  function renderTable () {
    ...
    thead.innerHTML = `
      <tr class="text-left text-[13px] font-medium text-slate-500">
        <th class="w-10 border-b border-slate-200 px-3 py-3 font-medium">
          <input type="checkbox" id="hd-select-all" class="rounded border-slate-300" />
        </th>
        <th class="w-12 border-b border-slate-200 px-2 py-3 font-medium">#</th>
        <th class="w-24 border-b border-slate-200 px-3 py-3 font-medium">状态</th>
        ${cols.map(c => `<th class="border-b border-slate-200 px-3 py-3 font-medium whitespace-nowrap">${c}</th>`).join('')}
        ${prompts.flatMap(p => [
          `<th class="border-b border-slate-200 px-3 py-3 font-medium whitespace-nowrap text-violet-600">[${p.role}]_${gtCol}</th>`,
          `<th class="border-b border-slate-200 px-3 py-3 font-medium whitespace-nowrap text-violet-400">[${p.role}]_thinking</th>`,
        ]).join('')}
        <th class="border-b border-slate-200 px-3 py-3 font-medium whitespace-nowrap text-emerald-700">GT (${gtCol})</th>
        <th class="sticky-action-col w-12 border-b border-slate-200 px-2 py-3"></th>
      </tr>
    `;
  ...
  function rowHTML (r, cols, prompts, gtCol) {
    ...
    const clsHint = cls ? `<span class="ml-1 text-[10px] text-slate-400 uppercase">${cls}</span>` : '';
    return `
      <tr class="tbody-row ${selected ? 'bg-emerald-50/40' : ''}" data-rid="${r.id}">
        ...
        <td class="border-b border-slate-100 px-2 py-2.5 text-slate-400">${r.no}</td>
        <td class="border-b border-slate-100 px-3 py-2.5">${statusCell(r)}${clsHint}</td>
        ${cols.map(c => `<td class="border-b border-slate-100 px-3 py-2.5 max-w-[260px] text-truncate">${escapeHTML(String(r.data[c] ?? ''))}</td>`).join('')}
        ${prompts.flatMap(p => [
          `<td class="border-b border-slate-100 px-3 py-2.5">${cellPred(r, p, gtCol)}</td>`,
          `<td class="border-b border-slate-100 px-3 py-2.5 max-w-[280px] text-truncate text-slate-500">${cellThinking(r, p)}</td>`,
        ]).join('')}
        <td class="border-b border-slate-100 px-3 py-2.5 font-medium text-emerald-700">${escapeHTML(String(r.data[gtCol] ?? '-'))}</td>
        <td class="sticky-action-col border-b border-slate-100 px-2 py-2.5 text-right">
          <button data-action="menu" class="inline-flex h-7 w-7 items-center justify-center rounded-md text-slate-500 hover:bg-slate-100" title="操作菜单">
  ...
  function cellPred (r, p, gtCol) {
    ...
    if (r.status === 'running') return `<span class="inline-flex items-center gap-1 text-amber-600 text-xs"><span class="spinner"></span>标注中…</span>`;
    if (r.status === 'failed') return `<span class="text-rose-600 text-xs">⚠ 失败</span>`;
  ...
  function openRowMenu (anchor, rid) {
    NX.openActionMenu(anchor, [
      { icon:'👁', label:'详情',     onClick: () => Page.openDrawer(rid, 'current') },
      { icon:'▶',  label:'标注',     onClick: () => simulateAnnotate([rid]) },
      { icon:'🕘', label:'标注历史', onClick: () => Page.openDrawer(rid, 'history') },
      { icon:'✎',  label:'编辑',     onClick: () => editRow(rid) },
      { icon:'🔍', label:'分析',     onClick: () => analyzeRow(rid) },
      { icon:'⬇',  label:'导出',     onClick: () => exportRow(rid) },
      '-',
  ...
  function simulateAnnotate (rowIds) {
    ...
    renderAll();
    NX.toast(`已开始标注 ${rowIds.length} 行(模拟,5~30 秒后返回)`, 'success');
    ...
    rowIds.forEach((rid, idx) => {
      ...
      setTimeout(() => {
        ...
        if (rand < 0.1) {
          ...
          const opts = [POS, NEG, NEU];
          const prompts = getCurrentPrompts();
          prompts.forEach(p => {
            const pred = Math.random() < 0.85 ? r.data['情感分类'] : opts[Math.floor(Math.random()*3)];
            r.results[`[${p.role}]_情感分类`] = pred;
            r.results[`[${p.role}]_thinking`] = `${p.role}基于反馈词义判断为${pred}。`;
          });
  ...
  function editRow (rid) { NX.toast('编辑行 — 原型简化:轻量行内编辑待实现'); }
  function analyzeRow (rid) {
    const r = state.rows.find(x => x.id === rid);
    if (!r) return;
    // mock UserHooks.analyze
    const result = {
      summary: `行 ${rid} 的分析摘要(mock)`,
      key_signals: ['用户反馈含情感词', 'GT 与预测一致'],
      score: 0.78,
      sentiment_breakdown: { 正: 0.6, 负: 0.2, 中: 0.2 },
    };
  ...
  function deleteRow (rid) {
    if (!confirm('确认删除该行?(原型:仅前端移除)')) return;
  ...
  Page.bulkDelete = function () {
    ...
    state.selected.clear();
    renderAll();
    NX.toast('已批量删除', 'success');
  };

  /* ───────── 添加错题弹窗 ───────── */
  Page.openAddError = function () {
    if (state.selected.size === 0) { NX.toast('请先勾选至少一行'); return; }
    ...
    const promptCols = getCurrentPrompts().flatMap(p => [
      `[${p.role}]_${gtCol}`,
      `[${p.role}]_thinking`,
    ]);
  ...
  function updateAddErrorPreview () {
    ...
    const obj = {};
    picked.forEach(c => {
      obj[c] = c.startsWith('[') ? first.results[c] : first.data[c];
    });
  ...
  Page.confirmAddError = function () {
    const picked = [...document.querySelectorAll('#ae-cols input:checked')].map(c => c.dataset.col);
    if (picked.length === 0) { NX.toast('请至少选一列', 'error'); return; }
    const ids = [...state.selected];
    NX.closeModal(document.getElementById('add-error-modal'));
    NX.toast(`已将 ${ids.length} 行加入错题本(原型:跳至「错题集管理」可见)`, 'success');
    // 实际原型:写入 NX.errorEntries
    ids.forEach(id => {
  ...
  Page.openDrawer = function (rid, defaultTab) {
    ...
    document.querySelectorAll('.drawer-tab').forEach(t => {
      const isActive = t.dataset.dtab === state.drawerTab;
  ...
  function renderDrawerBody () {
    ...
    if (state.drawerTab === 'current') {
      ...
      body.innerHTML = `
        ...
        <section>
          <div class="text-xs font-semibold text-slate-500 mb-2">🧩 渲染后 Prompt(示例,第一个角色)</div>
        ...
        ${prompts.map(p => `
          <section>
            ...
            <pre class="json-view">${NX.renderJSON({ thinking: r.results[`[${p.role}]_thinking`] || '-', [gtCol]: r.results[`[${p.role}]_${gtCol}`] || '-' })}</pre>
      ...
      body.innerHTML = hist.map((h, idx) => `
        <section class="rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-3">
          <div class="flex items-center justify-between">
            <div class="text-sm font-medium text-slate-800">${idx === 0 ? '🕒 最近' : '🕒'} ${h.time} · ${h.schemeName}</div>
          ...
          <div class="mt-2 space-y-1.5 text-xs">
            ${h.roles.map(role => `
              <div class="flex items-center gap-2">
                ...
                <span class="font-medium text-slate-800">${role.parsed['情感分类'] || '-'}</span>
                <span class="text-slate-400">· ${role.model} · ${(role.elapsedMs / 1000).toFixed(1)}s</span>
              </div>
            `).join('')}
            <div class="text-slate-500 mt-1">GT = <span class="text-emerald-700 font-medium">${h.gt}</span> · 结论:${verdictBadge(h.verdict)}</div>
          </div>
        </section>
      `).join('');
    }
  ...
  Page.drawerTranslate = function () {
    NX.toast('翻译(mock):返回 [已翻译·zh] 前缀文本(具体由 UserHooks.translate 实现)', 'default', 3000);
  };

  /* ───────── 方案管理 Tab ───────── */
  Page.newScheme = function () { NX.toast('原型:方案 CRUD 表单待补,核心数据结构已就绪'); };
  function renderSchemes () {
    document.getElementById('schemes-tbody').innerHTML = NX.schemes.map(s => {
      ...
      return `
        <tr class="tbody-row">
          <td class="border-b border-slate-100 px-4 py-3 font-medium">${s.name}</td>
          <td class="border-b border-slate-100 px-4 py-3"><span class="badge badge--sky">${s.scene}</span></td>
          <td class="border-b border-slate-100 px-4 py-3 text-slate-600">${roleList}</td>
          <td class="border-b border-slate-100 px-4 py-3 text-slate-600">${s.concurrency}</td>
          <td class="border-b border-slate-100 px-4 py-3 text-slate-500">${s.lastUsed}</td>
          <td class="border-b border-slate-100 px-4 py-3 text-right">
            <button class="text-sm font-medium text-emerald-600 hover:underline" onclick="NX.toast('原型:编辑方案表单待补')">编辑</button>
            <span class="text-slate-300 mx-2">|</span>
            <button class="text-sm font-medium text-rose-600 hover:underline" onclick="NX.toast('原型:删除方案待接')">删除</button>
          </td>
        </tr>
      `;
    }).join('');
  ...
  function renderTasks () {
    document.getElementById('tasks-tbody').innerHTML = NX.tasks.map(t => {
      ...
      const statusCell = t.status === 'running'
        ? `<div class="flex items-center gap-2">
             ...
             <span class="text-xs text-slate-500">${t.progress.done}/${t.progress.total} · ${pct}%</span>
           </div>`
        : t.status === 'done'
        ? `<span class="badge badge--green">✅ 已完成</span> <span class="ml-1 text-xs text-slate-500">准确率 ${(t.accuracy*100).toFixed(1)}%</span>`
        : t.status === 'failed'
        ? `<span class="badge badge--rose">❌ 失败</span> <span class="ml-1 text-xs text-slate-500">${t.errMsg || ''}</span>`
        : t.status === 'cancelled'
        ? `<span class="badge badge--slate">已取消</span>`
        : `<span class="badge badge--slate">${t.status}</span>`;
      return `
        <tr class="tbody-row">
          <td class="border-b border-slate-100 px-4 py-3 font-mono text-xs text-slate-600">${t.id}</td>
          <td class="border-b border-slate-100 px-4 py-3 text-slate-600">${t.triggeredAt}</td>
          <td class="border-b border-slate-100 px-4 py-3">${t.datasetName}</td>
          <td class="border-b border-slate-100 px-4 py-3"><span class="badge badge--sky">${t.schemeName}</span></td>
          <td class="border-b border-slate-100 px-4 py-3 text-slate-600">${t.rowCount}</td>
          <td class="border-b border-slate-100 px-4 py-3">${statusCell}</td>
          <td class="border-b border-slate-100 px-4 py-3 text-right">
            ${t.status === 'running'
              ? `<button class="text-sm font-medium text-rose-600 hover:underline" onclick="NX.toast('原型:取消任务流程待接')">取消</button>`
              : t.status === 'failed'
              ? `<button class="text-sm font-medium text-emerald-600 hover:underline" onclick="NX.toast('原型:重跑失败行待接')">重跑失败</button>`
              : `<button class="text-sm font-medium text-slate-600 hover:underline" onclick="NX.toast('原型:任务详情待接')">详情</button>`
            }
          </td>
        </tr>
      `;
