...
- 完成场景、数据集、Prompt、知识库、fewshots样例、标注方案的基础管理
...
| 2 | 数据集与方案管理 | 管理场景、数据集、Prompt、知识库、fewshots样例、标注方案 |
...
该页面用于完成第一阶段的资源准备工作，核心组织方式是「场景」。数据集、Prompt、知识库、fewshots样例都归属于某个场景。
...
fewshots样例用于保存标注过程中沉淀的错误样本。

第一阶段范围：

- 保留fewshots样例资源卡片和基础数据结构
- 支持列表展示和手动管理的接口预留
- 标注工作台“一键添加fewshots样例”功能暂不实现

### 5.6 卡片展示与弹窗

场景内容区以卡片形式展示资源：数据集、Prompt、知识库、fewshots样例。
...
- 编辑基础信息
- 编辑字段映射或模板内容
- 查看关联关系，例如 Prompt 引用了哪些知识库、fewshots样例属于哪个场景
...
- 自定义处理会把 Prompt、知识库、fewshots样例转换为 dict 后传给用户实现的方法
- 自定义处理入参结构：`prompt_contents={角色名: Prompt对象}`、`knowledge={知识名称: 知识内容}`、`error_sets={fewshots样例名称: fewshots样例内容}`
...
| 字段 | 类型 | 说明 |
|---|---|---|
| id | TEXT PRIMARY KEY | fewshots样例 ID |
| scene_id | TEXT NOT NULL | 所属场景 |
| name | TEXT NOT NULL | fewshots样例名称 |
| description | TEXT | 描述 |
| created_at | TEXT | 创建时间 |
| updated_at | TEXT | 更新时间 |
